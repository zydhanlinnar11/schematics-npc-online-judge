...
$(function () {
  ...
  function onchange(evt) {
    ...
    if (evt.type in evtMap) document.body.className = evtMap[evt.type]
      ...
      document.body.className = this[hidden]
        ? 'window-hidden'
        : 'window-visible'
...
$(function () {
  if (typeof window.orientation !== 'undefined') {
    $(window).resize(function () {
      ...
      $('#viewport').attr(
        'content',
        width > 480 ? 'initial-scale=1' : 'width=480'
      )
    })
  }
  ...
  $('#navicon')
    .click(function (event) {
      ...
      $nav_list.toggle()
      if ($nav_list.is(':hidden')) $(this).blur().removeClass('hover')
      else {
        $(this).addClass('hover')
...
function register_time(elems, limit) {
  ...
  elems.each(function () {
    ...
    function update() {
      if ($('body').hasClass('window-hidden')) return (outdated = true)
      outdated = false
...
window.notify = function (type, title, data, timeout) {
  ...
  var data =
    typeof data !== 'undefined' ? $.extend({}, template, data) : template
